...
export default {
  ...
  methods: {
    ...
    currentError() {
      ...
      if (!isValidName) {
        return "Constant name must be alphanumeric without spaces and cannot start with a number";
      }
      if (alreadyExists) {
        return "You have already defined a constant with this name";
      }
      if (hasCommandConflict) {
        return "Constant name conflicts with a command key word";
      }
      if (shadowsPrototype) {
        return "Constant name cannot shadow a built-in Javascript prototype prop";
      }

      if (!this.valueString) {
        return "Constant value cannot be empty";
      }
      ...
      if (!isNumber && !isStudyString) {
        return "Constant value must either be a number or Time Study string";
      }
      return null;
    },
    errorTooltip() {
    ...
    handleFocus(focus) {
      if (focus || this.currentError()) {
        return;
      }
      if (!this.aliasString) {
        AutomatorBackend.deleteConstant(this.oldAlias);
      } else if (!this.oldAlias) {
        AutomatorBackend.addConstant(this.aliasString, this.valueString);
      } else if (this.oldAlias === this.aliasString) {
        AutomatorBackend.modifyConstant(this.aliasString, this.valueString);
      } else {
        AutomatorBackend.renameConstant(this.oldAlias, this.aliasString);
      }
      this.oldAlias = this.aliasString;
