export const BlackHoleAnimation = (function () {
  ...
  class Particle extends Dot {
    ...
    drawTrail(context) {
      ...
      const { x: lastX, y: lastY } = Dot.calculatePosition(
        this.preLastDistance,
        angle,
      );
  ...
  return class Animation {
    ...
    totalPhase() {
      ...
      if (blackHole.isActive) {
        return blackHole.phase.sub(blackHole.duration.div(2)).add(this.period)
          .mod(this.period);
      }
