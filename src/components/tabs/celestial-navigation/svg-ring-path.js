...
${rMinorCircle}`;
  ...
  const gapAngleDeg = d.gapAngleDeg === undefined
    ? d.gapDeg / 2
    : d.gapAngleDeg;
  ...
  const x = d.rMajor / d.rMinor
    * Math.sin(toRad * (d.gapDeg / 2 - gapAngleDeg));
...
A ${d.rMajor} ${d.rMajor} 0 ${big} 1 ${c1 * d.rMajor + 1e-3 * s1} ${
  s1 * d.rMajor - 1e-3 * c1
}
L ${c3 * d.rMinor + 1e-3 * s3} ${s3 * d.rMinor - 1e-3 * c3}
A ${d.rMinor} ${d.rMinor} 0 ${big} 0 ${c2 * d.rMinor - 1e-3 * s2} ${
  s2 * d.rMinor + 1e-3 * c2
}
z`;
}
