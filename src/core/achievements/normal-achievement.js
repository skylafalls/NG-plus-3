...
class AchievementState extends GameMechanicState {
  ...
  unlock(auto) {
    ...
    if (
      player.speedrun.isActive && !player.speedrun.achievementTimes[this.id]
    ) {
      ...
      player.speedrun.achievementTimes[this.id] = Math.floor(
 ...
 */
export const Achievement = AchievementState.createAccessor(
...
export const Achievements = {
  ...
  autoAchieveUpdate(diff) {
    ...
    if (!player.reality.autoAchieve || RealityUpgrade(8).isLockingMechanics) {
      player.reality.achTimer = Decimal.clampMax(
  ...
  _power: new Lazy(() => {
    ...
    const exponent = getAdjustedGlyphEffect("effarigachievement").mul(
...
EventHub.logic.on(GAME_EVENT.PERK_BOUGHT, () => {
  player.reality.achTimer = Decimal.min(
    player.reality.achTimer,
    Achievements.period,
  );
});
