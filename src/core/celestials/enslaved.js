...
export const ENSLAVED_UNLOCKS = {
  FREE_TICKSPEED_SOFTCAP: {
    ...
    description: () =>
      `Increase the softcap to Tickspeed upgrades from Time Dimensions by ${
        formatInt(1e5)
      }`,
  },
  RUN: {
    ...
    secondaryRequirement() {
      const hasLevelRequirement = player.records.bestReality.glyphLevel.gte(
        5000,
      );
    ...
    description() {
      ...
      return `Unlock The Nameless Ones' Reality (requires ${
        hasLevelRequirement ? "[✓]" : "[✗]"
      } a level
      ${formatInt(5000)} Glyph and ${hasRarityRequirement ? "[✓]" : "[✗]"} a ${
        formatRarity(new Decimal(100))
      } rarity Glyph)`;
    },
  },
};
...
export const Enslaved = {
  ...
  toggleStoreBlackHole() {
    if (!this.canModifyGameTimeStorage) {
      return;
    }
    player.celestials.enslaved.isStoring = !player.celestials.enslaved
      .isStoring;
    player.celestials.enslaved.isStoringReal = false;
  },
  toggleStoreReal() {
    if (!this.canModifyRealTimeStorage && !this.isStoredRealTimeCapped) {
      return;
    }
    player.celestials.enslaved.isStoringReal = !player.celestials.enslaved
      .isStoringReal;
    player.celestials.enslaved.isStoring = false;
  },
  toggleAutoStoreReal() {
    if (!this.canModifyRealTimeStorage) {
      return;
    }
    player.celestials.enslaved.autoStoreReal = !player.celestials.enslaved
      .autoStoreReal;
  },
  ...
  get storedRealTimeCap() {
    const addedCap = Ra.unlocks.improvedStoredTime.effects.realTimeCap
      .effectOrDefault(0);
  ...
  storeRealTime(diffVal) {
    ...
    const diff = diffVal === undefined
      ? Decimal.max(thisUpdate - player.lastUpdate, 0)
      : diffVal;
    ...
    player.celestials.enslaved.storedReal = player.celestials.enslaved
      .storedReal.add(diff.mul(efficiency));
  ...
  useStoredTime(autoRelease) {
    if (!this.canRelease(autoRelease)) {
      return;
    }
    ...
    if (
      ...
      if (!autoRelease) {
        ImaginaryUpgrade(24).tryShowWarningModal("discharge your Black Hole");
      }
      return;
    }
    ...
    if (Enslaved.isRunning) {
      ...
      if (
        Time.thisReality.totalYears.add(
          TimeSpan.fromMilliseconds(release).totalYears.gt(1),
        )
      ) {
    ...
    player.celestials.ra.peakGamespeed = Decimal.max(
      player.celestials.ra.peakGamespeed,
      effectiveGamespeed,
    );
    this.autoReleaseSpeed = release.div(player.options.updateRate).div(5);
    player.celestials.enslaved.stored = player.celestials.enslaved.stored.mul(
      autoRelease ? 0.99 : 0,
    );
  },
  has(info) {
  ...
  buyUnlock(info) {
    ...
    if (info.id === ENSLAVED_UNLOCKS.RUN.id) {
      this.quotes.unlockRun.show();
    }
    player.celestials.enslaved.stored = player.celestials.enslaved.stored.sub(
      info.price,
    );
  ...
  initializeRun() {
    ...
    if (
      ...
      GameUI.notify.error(
        "This Reality forbids Black Holes! (Automator stopped)",
      );
    }
  ...
  get realityBoostRatio() {
    return Decimal.max(
      1,
      Decimal.floor(player.celestials.enslaved.storedReal
  ...
  storedTimeInsideEnslaved(stored) {
    ...
    return Decimal.pow(10, Decimal.pow(Decimal.log10(stored.div(1e3)), 0.55))
      .times(1e3);
  },
  feelEternity() {
  ...
  get nextHintCost() {
    return TimeSpan.fromYears(
      new Decimal(1e40 * Math.pow(3, this.hintCostIncreases)),
    ).totalMilliseconds;
  },
  get hintCostIncreases() {
    const hintTime = player.celestials.enslaved.zeroHintTime - Date.now();
    return Math.clampMin(
      hintTime / TimeSpan.fromDays(new Decimal(1)).totalMilliseconds.toNumber(),
      0,
    );
  },
  spendTimeForHint() {
    if (player.celestials.enslaved.stored < this.nextHintCost) {
      return false;
    }
...
export const Tesseracts = {
  ...
  buyTesseract() {
    if (!this.canBuyTesseract) {
      return;
    }
    if (GameEnd.creditsEverClosed) {
      return;
    }
    player.celestials.enslaved.tesseracts = player.celestials.enslaved
      .tesseracts.add(1);
  },
  ...
  costs(index) {
    index = index.add(1);
    if (index.lte(3)) {
      return Decimal.pow10(index.times(2e7));
    }
    return Decimal.pow10(
      (index.sub(3)).factorial().times(Decimal.pow(2, index.sub(3))).times(6e7),
    );
  },
  ...
  capIncrease(count = this.bought) {
    const totalCount = count.times(
      SingularityMilestone.tesseractMultFromSingularities.effectOrDefault(1),
    );
    const base = totalCount.lt(1)
      ? DC.D0
      : Decimal.pow(2, totalCount).times(2.5e5);
    return base.times(DC.D1.add(AlchemyResource.boundless.effectValue));
  },

  get nextTesseractIncrease() {
    return this.capIncrease(this.bought.add(1)).sub(
      this.capIncrease(this.bought),
    );
  },
};

EventHub.logic.on(GAME_EVENT.TAB_CHANGED, () => {
  if (Tab.celestials.enslaved.isOpen) {
    Enslaved.quotes.initial.show();
  }
});
