...
export const GameEnd = {
  get endState() {
    if (this.removeAdditionalEnd || player.bypassEnd) {
      return this.additionalEnd;
    }
    return Math.max(
      player.celestials.pelle.records.totalAntimatter.add(1).log10().add(1)
        .log10().sub(8.7)
  ...
  _additionalEnd: 0,
  get additionalEnd() {
    return (player.isGameEnd || this.removeAdditionalEnd)
      ? this._additionalEnd
      : 0;
  },
  set additionalEnd(x) {
    this._additionalEnd = (player.isGameEnd || this.removeAdditionalEnd)
      ? x
      : 0;
  },
