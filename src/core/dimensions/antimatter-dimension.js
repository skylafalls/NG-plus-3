...
export function getDimensionFinalMultiplierUncached(tier) {
  if (tier < 1 || tier > 8) {
    throw new Error(`Invalid Antimatter Dimension tier ${tier}`);
  }
  if (NormalChallenge(10).isRunning && tier > 6) {
    return DC.D1;
  }
...
function applyNDMultipliers(mult, tier) {
  let multiplier = mult.times(
    GameCache.antimatterDimensionCommonMultiplier.value,
  );
  ...
  multiplier = multiplier.times(
    Decimal.pow(AntimatterDimensions.buyTenMultiplier, buy10Value),
  );
...
function applyNDPowers(mult, tier) {
  ...
  multiplier = multiplier.pow(
    glyphPowMultiplier.times(glyphEffarigPowMultiplier).times(Ra.momentumValue),
  );
...
function onBuyDimension(tier) {
  ...
  if (player.speedrun.isActive && !player.speedrun.hasStarted) {
    Speedrun.startTimer();
  }

  if (NormalChallenge(2).isRunning) {
    player.chall2Pow = DC.D0;
  }
...
export function buyOneDimension(tier) {
  ...
  if (
    tier === 8 && Enslaved.isRunning && AntimatterDimension(8).bought.gte(1)
  ) {
    return false;
  }
...
export function buyMaxDimension(tier, bulk = Infinity) {
  ...
  if (dimension.cost.gt(goal) && Player.isInAntimatterChallenge) {
    return;
  }
  ...
  if (NormalChallenge(9).isRunning || InfinityChallenge(5).isRunning) {
    while (
      dimension.isAffordableUntil10 && dimension.cost.lt(goal) && bulkLeft.gt(0)
    ) {
...
class AntimatterDimensionState extends DimensionState {
  constructor(tier) {
    ...
    const C6_BASE_COST_MULTIPLIERS = [
      null,
      DC.E3,
      DC.E3.times(5),
      DC.E4,
  ...
  get costScale() {
    return new ExponentialCostScaling({
      baseCost: NormalChallenge(6).isRunning
        ? this._c6BaseCost
        : this._baseCost,
      baseIncrease: NormalChallenge(6).isRunning
  ...
  set currencyAmount(value) {
    if (this.tier >= 3 && NormalChallenge(6).isRunning) {
  ...
  get continuumValue() {
    ...
    if (this.tier === 8 && Enslaved.isRunning) {
      return DC.D1;
    }
  ...
  get isAffordable() {
    ...
    if (!player.break && this.cost.gt(DC.NUMMAX)) {
      return false;
    }
  ...
  challengeCostBump() {
    if (InfinityChallenge(5).isRunning) {
      this.multiplyIC5Costs();
    } else if (NormalChallenge(9).isRunning) {
      this.multiplySameCosts();
    }
  }
  ...
  multiplySameCosts() {
    ...
    if (Tickspeed.cost.e === this.cost.e) {
      chall9TickspeedCostBumps = chall9TickspeedCostBumps.add(1);
    }
  }
  ...
  get productionPerSecond() {
    ...
    if (tier === 1) {
      ...
      if (production.gt(10)) {
        ...
        production = Decimal.pow10(
          Decimal.pow(log10, getAdjustedGlyphEffect("effarigantimatter")),
        );
      }
    }
...
export const AntimatterDimensions = {
  ...
  tick(diff) {
    ...
    if (hasBigCrunchGoal && Currency.antimatter.gte(Player.infinityGoal)) {
      return;
    }
    ...
    if (hasBigCrunchGoal) {
      Currency.antimatter.dropTo(Player.infinityGoal);
    }
  },
};
