...
export function buySingleTimeDimension(tier, auto = false) {
  ...
  if (tier > 4) {
    if (!TimeStudy.timeDimension(tier).isBought) {
      return false;
    }
    if (
  ...
  if (Currency.eternityPoints.lt(dim.cost)) {
    return false;
  }
  if (Enslaved.isRunning && dim.bought.gt(0)) {
    return false;
  }
  if (
...
export function calcHighestPurchaseableTD(tier, currency) {
  ...
  if (currency.gte(DC.E6000)) {
    logMult = TimeDimension(tier)._costMultiplier.mul(tier <= 4 ? 2.2 : 1).max(
      1,
    ).log10();
    const preInc = Decimal.log10(DC.E6000).sub(logBase).div(logMult);
    const postInc = logC.sub(logBase).sub(6000).div(logMult).div(
      TimeDimensions.scalingPast1e6000,
    ).clampMin(0);
...
export function buyMaxTimeDimension(
  ...
  if (Enslaved.isRunning) {
    return buySingleTimeDimension(tier);
  }
  const pur = Decimal.sub(calcHighestPurchaseableTD(tier, canSpend), dim.bought)
    .clampMin(0);
...
export function maxAllTimeDimensions() {
  ...
  for (let stop = 0; stop < 1000; stop++) {
    ...
    if (!buySingleTimeDimension(cheapestDim.tier, true)) {
      break;
    }
  }
}
...
export function timeDimensionCommonMultiplier() {
  ...
  if (EternityChallenge(9).isRunning) {
    mult = mult.times(
      Decimal.pow(
        Decimal.clampMin(
          Currency.infinityPower.value.max(1).pow(
            InfinityDimensions.powerConversionRate.div(7),
          ).log2(),
          1,
        ),
        4,
...
class TimeDimensionState extends DimensionState {
  ...
  nextCost(bought) {
    ...
    for (let i = 0; i < this._costIncreaseThresholds.length; i++) {
      ...
      if (cost.lt(this._costIncreaseThresholds[i])) {
        return cost;
      }
    }
    ...
    const exponent = this.e6000ScalingAmount.add(
      (bought.sub(this.e6000ScalingAmount))
...
export const TimeDimensions = {
  ...
  tick(diff) {
    ...
    for (let tier = maxTierProduced; tier >= 1; --tier) {
      TimeDimension(tier + nextTierOffset).produceDimensions(
        TimeDimension(tier),
        diff.div(10),
      );
    }
    ...
    if (EternityChallenge(7).isRunning) {
      TimeDimension(1).produceDimensions(
        InfinityDimension(QuantumChallenge(4).isRunning ? 7 : 8),
        diff,
      );
    } else {
...
export function tryUnlockTimeDimensions() {
  if (TimeDimension(8).isUnlocked) {
    return;
  }
  for (let tier = 5; tier <= 8; ++tier) {
    if (TimeDimension(tier).isUnlocked) {
      continue;
    }
