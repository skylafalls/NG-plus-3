...
Decimal.prototype.expEquiv = function (decimal, floor = false) {
  if (decimal.abs().lte(1)) {
    return new Decimal(0);
  }
  if (!floor) {
    return decimal.max(1).log10();
  }
  return decimal.max(1).log10().floor();
};

Decimal.prototype.logPow = function (decimal) {
  let value = Decimal.fromValue_noAlloc(this).log10();
  value = value.pow(decimal);
  return Decimal.pow10(value);
};

Decimal.prototype.logRoot = function (decimal) {
  let value = Decimal.fromValue_noAlloc(this).log10();
  value = value.root(decimal);
  return Decimal.pow10(value);
};
