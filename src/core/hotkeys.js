...
function keyboardPressEscape() {
  if (Quote.isOpen || Quote.isHistoryOpen) {
    Quote.clearAll();
...
function keyboardEditAutobuyers() {
  ...
  if (!Autobuyers.hasAutobuyersForEditModal) {
    return;
  }
  if (Modal.isOpen) {
    Modal.hideAll();
...
function testKonami(character) {
  if (SecretAchievement(17).isUnlocked) {
    return;
  }
  ...
  if (konamiCode[konamiStep] === character) {
    konamiStep++;
  } else if (konamiStep === 2 && character === "up") {
    konamiStep = 2;
  } else if (character === konamiCode[0]) {
    konamiStep = 1;
  } else {
    konamiStep = 0;
  }
