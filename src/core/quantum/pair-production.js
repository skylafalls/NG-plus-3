...
class ElectronsUpgradeState extends RebuyableMechanicState {
  constructor(config) {
    config.effect = () =>
      this.boughtAmount.times(this.config.multiplierIncrement());
    super(config);
  }
  ...
  get bulk() {
    return this.currency.max(1).cbrt().log(this.config.increment).div(
...
export const ElectronUpgrade = ElectronsUpgradeState.createAccessor(
  GameDatabase.quantum.electronMultiplierUpgrades,
);
export const PositronUpgrade = PositronsUpgradeState.createAccessor(
  GameDatabase.quantum.positronMultiplierUpgrades,
);
...
export const PairProduction = {
  ...
  sacrificeGalaxies() {
    ...
    const sacrificableGalaxies = player.galaxies.sub(
      player.quantum.pair.dischargedGalaxies,
    );
    const gainedPositrons = sacrificableGalaxies.mul(this.positronMultiplier)
      .max(0);
    const gainedElectrons = sacrificableGalaxies.mul(this.electronMultiplier)
      .max(0);

    player.quantum.pair.dischargedGalaxies = player.galaxies;
    player.quantum.pair.positrons = player.quantum.pair.positrons.plus(
      gainedPositrons,
    );
    player.quantum.pair.electrons = player.quantum.pair.electrons.plus(
      gainedElectrons,
    );
    return true;
  },
  ...
  updateDischarges() {
    const gainedPositrons = player.quantum.pair.dischargedGalaxies.mul(
      this.positronMultiplier,
    ).max(0);
    const gainedElectrons = player.quantum.pair.dischargedGalaxies.mul(
      this.electronMultiplier,
    ).max(0);

    player.quantum.pair.positrons = player.quantum.pair.positrons.max(
      gainedPositrons,
    );
    player.quantum.pair.electrons = player.quantum.pair.electrons.max(
      gainedElectrons,
    );
  },
};
