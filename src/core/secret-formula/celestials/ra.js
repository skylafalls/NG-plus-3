export const ra = {
  pets: {
    teresa: {
      ...
      rawMemoryChunksPerSecond: () =>
        Decimal.pow(Currency.eternityPoints.value.add(1).log10().div(1e4), 3)
          .mul(4),
      memoryProductionMultiplier: () =>
        Ra.unlocks.teresaXP.effectOrDefault(new Decimal(1)),
    },
    effarig: {
      id: "effarig",
      name: "Effarig",
      ...
      rawMemoryChunksPerSecond: () =>
        Decimal.pow(Effarig.shardsGained, 0.1).mul(4),
      memoryProductionMultiplier: () =>
        Ra.unlocks.effarigXP.effectOrDefault(new Decimal(1)),
    },
    enslaved: {
      id: "enslaved",
      ...
      rawMemoryChunksPerSecond: () =>
        Decimal.pow(Currency.timeShards.value.add(1).log10().div(3e5), 2).mul(
          4,
        ),
      memoryProductionMultiplier: () =>
        Ra.unlocks.enslavedXP.effectOrDefault(new Decimal(1)),
    },
    v: {
      id: "v",
      name: "V",
      ...
      rawMemoryChunksPerSecond: () =>
        Decimal.pow(Currency.infinityPower.value.add(1).log10().div(1e7), 1.5)
          .mul(4),
      memoryProductionMultiplier: () =>
  ...
  unlocks: {
    ...
    teresaXP: {
      id: 2,
      reward:
        "All Memory Chunks produce more Memories based on Reality Machines",
      effect: () =>
        Decimal.pow(
          Currency.realityMachines.value.add(1).log10().div(100),
          0.5,
        ),
      pet: "teresa",
      level: 5,
    ...
    unlockDilationStartingTP: {
      id: 6,
      reward:
        `In non-Celestial Realities, gain Tachyon Particles as if you reached the square root of your total
    ...
    extraGlyphChoicesAndRelicShardRarityAlwaysMax: {
      ...
      reward: () =>
        `Get ${
          formatX(2)
        } Glyph choices and the bonus to Glyph rarity from Relic Shards
    ...
    unlockGlyphAlchemy: {
      id: 8,
      reward:
        `Unlock Glyph Alchemy, which adds alchemical resources you can increase by Refining Glyphs. You unlock
    ...
    effarigXP: {
      id: 9,
      reward:
        "All Memory Chunks produce more Memories based on highest Glyph level",
    ...
    relicShardGlyphLevelBoost: {
      ...
      effect: () =>
        Decimal.pow(Decimal.log10(Decimal.max(Effarig.shardsGained, 1)), 2).mul(
          100,
        ),
      pet: "effarig",
      level: 15,
    ...
    maxGlyphRarityAndShardSacrificeBoost: {
      id: 13,
      reward: () =>
        `Glyphs are always generated with ${formatPercents(1)} rarity and
    ...
    improvedStoredTime: {
      id: 15,
      reward:
        "Stored game time is amplified and you can store more real time, increasing with Nameless levels",
      effects: {
        gameTimeAmplification: () =>
          Decimal.pow(20, Math.clampMax(Ra.pets.enslaved.level, Ra.levelCap)),
    ...
    enslavedXP: {
      id: 16,
      reward:
        "All Memory Chunks produce more Memories based on total time played",
      effect: () =>
    ...
    autoPulseTime: {
      id: 17,
      reward: () =>
        `Black Hole charging now only uses ${
          formatPercents(0.99)
        } of your game speed and you can
    ...
    peakGamespeedDT: {
      ...
      effect: () =>
        Decimal.max(
          Decimal.pow(
            Decimal.log10(player.celestials.ra.peakGamespeed).sub(90),
            3,
          ),
          1,
        ),
    ...
    allGamespeedGlyphs: {
      id: 20,
      reward:
        `All basic Glyphs gain the increased game speed effect from Time Glyphs,
    ...
    autoUnlockDilation: {
      id: 22,
      reward: () =>
        `In non-Celestial Realities, Time Dilation is unlocked automatically for free at
        ${
    ...
    vXP: {
      id: 23,
      reward:
        "All Memory Chunks produce more Memories based on total Celestial levels.",
    ...
    continuousTTBoost: {
      id: 25,
      reward:
        "Time Theorems boost all forms of continuous non-dimension production",
      effects: {
