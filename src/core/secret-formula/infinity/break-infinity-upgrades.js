...
function rebuyable(config) {
  ...
  return {
    ...
    id,
    cost: () =>
      config.initialCost.mul(
        Decimal.pow(config.costIncrease, player.infinityRebuyables[config.id]),
      ),
    maxUpgrades,
    description,
    ...
    formatEffect: config.formatEffect ||
      ((value) => {
        ...
        return value.gte(config.maxUpgrades)
          ...
          : `Currently: ${formatX(DC.E1.sub(value))} | Next: ${
...
export const breakInfinityUpgrades = {
  ...
  slowestChallengeMult: {
    ...
    description:
      "Antimatter Dimensions gain a multiplier based on how fast your slowest challenge run is",
    effect: () =>
      Decimal.clampMin(
  ...
  infinitiedGen: {
    ...
    formatEffect: (value) => {
      ...
      const timeStr = Time.bestInfinity.totalMilliseconds.lte(50)
        ? `${
          TimeSpan.fromMilliseconds(new Decimal(100)).toStringShort()
        } (capped)`
  ...
  dimCostMult: rebuyable({
    ...
    description:
      "Reduce post-infinity Antimatter Dimension cost multiplier scaling",
  ...
  ipGen: rebuyable({
    ...
    description: () => {
      ...
      if (!BreakInfinityUpgrade.ipGen.isCapped) {
        generation += ` ➜ ${
          format(player.infinityRebuyables[2].add(1).mul(5))
        }%`;
      }
