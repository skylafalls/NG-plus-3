...
export const infinityUpgrades = {
  ...
  resetBoost: {
    ...
    description: () =>
      `Decrease the number of Dimensions needed for Dimension Boosts and Antimatter Galaxies by ${
        formatInt(9)
      }`,
    effect: 9,
    charged: {
      description: () =>
        "Decrease Dimension Boost requirement based on Teresa level",
  ...
  buy10Mult: {
    ...
    charged: {
      description: () =>
        `The multiplier for buying ${
          formatInt(10)
        } Antimatter Dimensions gains `
  ...
  thisInfinityTimeMult: {
    ...
    effect: () =>
      Decimal.max(Decimal.pow(Time.thisInfinity.totalMinutes.div(4), 0.25), 1),
    formatEffect: value => formatX(value, 2, 2),
    charged: {
      description:
      ...
      effect: () =>
        Decimal.log10(
          Decimal.log10(Time.thisInfinity.totalMilliseconds.add(100)),
        )
  ...
  unspentIPMult: {
    ...
    description:
      "Multiplier to 1st Antimatter Dimension based on unspent Infinity Points",
    effect: () => Currency.infinityPoints.value.dividedBy(2).pow(1.5).plus(1),
    formatEffect: value => formatX(value, 2, 2),
    charged: {
      description:
        "Multiplier to 1st Antimatter Dimension based on unspent Infinity Points, powered by Teresa level",
      effect: () =>
        Currency.infinityPoints.value.dividedBy(2).pow(
          Math.sqrt(Ra.pets.teresa.level) * 1.5,
        ).plus(1),
      formatEffect: value => formatX(value, 2, 2),
    },
  },
  dimboostMult: {
    ...
    charged: {
      description:
        "Dimension Boost multiplier gains a power effect based on Teresa level",
      effect: () => 1 + Ra.pets.teresa.level / 200,
      formatEffect: value => formatPow(value, 3, 3),
    },
  },
  ipGen: {
    ...
    description: () =>
      `Passively generate Infinity Points ${
        formatInt(10)
      } times slower than your fastest Infinity`,
    ...
    formatEffect: (value) => {
      if (Teresa.isRunning || V.isRunning) {
        return "Disabled in this reality";
      }
      if (Pelle.isDoomed) {
        return "Disabled";
      }
      if (player.records.bestInfinity.time.gte(DC.BEMAX.log10())) {
        return "Too slow to generate";
      }
      return `${format(value, 2)} every ${
        Time.bestInfinity.times(DC.E1).toStringShort()
      }`;
    },
    charged: {
      description: () =>
  ...
  skipReset1: {
    ...
    cost: 20,
    description: () =>
      `Start every reset with ${
        formatInt(1)
      } Dimension Boost, automatically unlocking the 5th Antimatter Dimension`,
  },
  skipReset2: {
    id: "skipReset2",
    cost: 40,
    ...
    description: () =>
      `Start every reset with ${
        formatInt(2)
      } Dimension Boosts, automatically unlocking the 6th Antimatter Dimension`,
  },
  skipReset3: {
    id: "skipReset3",
    cost: 80,
    ...
    description: () =>
      `Start every reset with ${
        formatInt(3)
      } Dimension Boosts, automatically unlocking the 7th Antimatter Dimension`,
  },
  skipResetGalaxy: {
    id: "skipResetGalaxy",
    cost: 300,
    ...
    description: () =>
      `Start every reset with ${
        formatInt(4)
      } Dimension Boosts, automatically unlocking the 8th Antimatter Dimension;
  ...
  ipOffline: {
    ...
    description:
      () => (player.options.offlineProgress
        ? `Only while offline, gain ${
          formatPercents(0.5)
        } of your best IP/min without using Max All`
    ...
    effect:
      () => (player.options.offlineProgress
        ? player.records.thisEternity.bestIPMsWithoutMaxAll.times(
            TimeSpan.fromMinutes(1).totalMilliseconds.div(2),
          )
        : DC.D0),
  ...
  ipMult: {
    ...
    checkRequirement: () => Achievement(41).isUnlocked,
    get costCap() {
      return MasteryStudy(11).isBought ? DC.BEMAX : DC.E6E6;
    },
    costIncreaseThreshold: DC.E3E6,
    description: () =>
      `Multiply Infinity Points from all sources by ${formatX(Effects.max(DC.D2, GluonUpgrade.greenBlue(3)))}`,
    // Normally the multiplier caps at e993k or so with 3300000 purchases, but if the cost is capped then we just give
    // an extra e7k to make the multiplier look nice
    effect: () => Effects.max(DC.D2, GluonUpgrade.greenBlue(3)).pow(player.IPMultPurchases),
    cap: () =>
      Effarig.eternityCap ?? (MasteryStudy(11).isBought ? DC.BEMAX : DC.E1E6),
    formatEffect: value => formatX(value, 2, 2),
  },
};
