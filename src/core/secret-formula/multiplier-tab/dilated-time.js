...
export const DT = {
  ...
  glyph: {
    name: "Glyph Effects",
    multValue: () => {
      const dtMult = getAdjustedGlyphEffect("dilationDT").times(
        Pelle.specialGlyphEffect.dilation,
      );
    ...
    isActive: () =>
      PlayerProgress.realityUnlocked() && getDilationGainPerSecond().neq(0),
    icon: MultiplierTabIcons.GENERIC_GLYPH,
  },
  ra1: {
    name: "Ra Upgrade - Multiplier based on TT",
    multValue: () =>
      DC.D1.timesEffectsOf(Ra.unlocks.continuousTTBoost.effects.dilatedTime),
    isActive: () =>
      Ra.unlocks.autoTP.canBeApplied && getDilationGainPerSecond().neq(0),
  ...
  gamespeed: {
    ...
    isActive: () =>
      getGameSpeedupForDisplay() > 1 && getDilationGainPerSecond().neq(0),
