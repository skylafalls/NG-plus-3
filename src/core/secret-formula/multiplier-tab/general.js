...
export const general = {
  achievement: {
    ...
    multValue: (ach, dim) => {
      ...
      if (ach === 141) {
        return Achievement(141).canBeApplied
          ? Achievement(141).effects.ipGain.effectOrDefault(1)
          : 1;
      }
      if (ach === 183) {
        return 1;
      ...
      if (!dim) {
        return Achievement(ach).canBeApplied
      ...
      if (dim?.length === 2) {
        ...
        for (
          let tier = 1;
          tier <= MultiplierTabHelper.activeDimCount(dim);
          tier++
        ) {
          let singleEffect;
          if (ach === 43) {
            ...
            singleEffect
              = (MultiplierTabHelper.achievementDimCheck(ach, `${dim}${tier}`)
      ...
      if (ach === 43) {
        return Achievement(43).canBeApplied
          ? (1 + Number(dim.charAt(2)) / 100)
          : 1;
      }
      return (MultiplierTabHelper.achievementDimCheck(ach, dim)
        && Achievement(ach).canBeApplied)
        ? Achievement(ach).effectOrDefault(1)
        : 1;
    },
  ...
  timeStudy: {
    name: (
      ts,
      dim,
    ) => (dim?.length === 2 ? `Time Study ${ts} (${dim})` : `Time Study ${ts}`),
    multValue: (ts, dim) => {
      ...
      if (!dim) {
        return TimeStudy(ts).canBeApplied
      ...
      if (dim?.length === 2) {
        ...
        for (
          ...
          totalEffect = totalEffect.times(
            (MultiplierTabHelper.timeStudyDimCheck(ts, `${dim}${tier}`)
      ...
      return (MultiplierTabHelper.timeStudyDimCheck(ts, dim)
        && TimeStudy(ts).isBought)
  ...
  infinityChallenge: {
    ...
    displayOverride: ic => (ic === 4
      ? formatPow(InfinityChallenge(4).reward.effectValue, 0, 3)
      : ""),
    multValue: (ic, dim) => {
      ...
      if (ic === 4) {
        ...
        const mults = AntimatterDimensions.all.map(ad =>
          ad.multiplier.pow((ic4Pow - 1) / ic4Pow),
        );
        if (dim?.length === 2) {
      ...
      if (dim?.length === 2) {
        ...
        for (
          ...
          totalEffect = totalEffect.times(
            (MultiplierTabHelper.ICDimCheck(ic, `${dim}${tier}`)
      ...
      if (ic === 8) {
        return (num > 1 && num < 8)
          ? InfinityChallenge(ic).reward.effectValue
          : DC.D1;
      }
    ...
    multValue: (ec, dim) => {
      if (dim?.length === 2) {
        ...
        for (
          ...
          totalEffect = totalEffect.times(
            (MultiplierTabHelper.ECDimCheck(ec, `${dim}${tier}`)
              && EternityChallenge(ec).reward.canBeApplied)
