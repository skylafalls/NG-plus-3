...
export const TD = {
  total: {
    name: (dim) => {
      if (dim) {
        return `TD ${dim} Multiplier`;
      }
      if (EternityChallenge(7).isRunning) {
        return "ID8 Production";
      }
  ...
  highestDim: {
    ...
    multValue: () =>
      TimeDimension(MultiplierTabHelper.activeDimCount("TD")).amount,
    ...
    multValue: (dim) => {
      const getMult = td =>
        Decimal.pow(
          4,
          td === 8
            ? Math.clampMax(TimeDimension(td).bought, 1e8)
            : TimeDimension(td).bought,
        );
      if (dim) {
        return getMult(dim);
      }
  ...
  achievement: {
    ...
    isActive: () =>
      Achievement(105).canBeApplied || Achievement(128).canBeApplied,
    icon: MultiplierTabIcons.ACHIEVEMENT,
  },
  timeStudy: {
    ...
    multValue: (dim) => {
      ...
      for (
        let tier = 1;
        tier <= MultiplierTabHelper.activeDimCount("TD");
        tier++
      ) {
    ...
    multValue: (dim) => {
      ...
      if (EternityChallenge(9).isRunning) {
        allMult = allMult.times(
          Decimal.pow(
            Math.clampMin(
              Currency.infinityPower.value.pow(
                InfinityDimensions.powerConversionRate / 7,
              )
                .log2(),
              1,
            ),
  ...
  tickspeed: {
    ...
    multValue: () =>
      Tickspeed.perSecond.pow(MultiplierTabHelper.activeDimCount("TD")),
