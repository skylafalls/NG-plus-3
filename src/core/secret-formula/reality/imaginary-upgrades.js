...
const rebuyable = (props) => {
  props.cost = () =>
    Decimal.pow(props.costMult, player.reality.imaginaryRebuyables[props.id])
      .times(props.initialCost);
  const { effect } = props;
  if (props.isDecimal) {
    props.effect = () =>
      Decimal.pow(effect, player.reality.imaginaryRebuyables[props.id]);
  } else {
    props.effect = () =>
      player.reality.imaginaryRebuyables[props.id].times(effect);
  }
...
export const imaginaryUpgrades = [
  rebuyable({
    ...
    costMult: 60,
    description: () =>
      `Increase Temporal Amplifier multiplier by +${format(0.15, 2, 2)}`,
    effect: 0.15,
  }),
  rebuyable({
    ...
    costMult: 60,
    description: () =>
      `Increase Replicative Amplifier multiplier by +${format(0.15, 2, 2)}`,
    effect: 0.15,
  }),
  rebuyable({
    ...
    costMult: 40,
    description: () =>
      `Increase Eternal Amplifier multiplier by +${format(0.4, 2, 2)}`,
    effect: 0.4,
  }),
  rebuyable({
    ...
    costMult: 80,
    description: () =>
      `Increase Superluminal Amplifier multiplier by +${format(0.15, 2, 2)}`,
    effect: 0.15,
  }),
  rebuyable({
    ...
    costMult: 30,
    description: () =>
      `Increase Boundless Amplifier multiplier by +${format(0.6, 2, 2)}`,
    effect: 0.6,
  }),
  rebuyable({
  ...
  rebuyable({
    ...
    costMult: 500,
    description: () =>
      `Delay Glyph Instability starting level by ${formatInt(200)}`,
    effect: 200,
  ...
  }),
  {
    ...
    effect: () =>
      player.records.totalAntimatter.max(1).log10().max(1).log10().div(100).add(
        1,
      ),
  ...
  },
  {
    ...
    requirement: () =>
      `Make a level ${
        formatInt(9000)
      } Glyph with a single Glyph level factor weight at
    ${formatInt(100)}`,
    hasFailed: () => false,
    checkRequirement: () =>
      Object.values(player.celestials.effarig.glyphWeights).some(w =>
        w === 100,
      )
  ...
  },
  {
    ...
    checkRequirement: () =>
      Enslaved.isRunning
      && MachineHandler.uncappedRM.times(simulatedRealityCount(false).add(1)).gte(
        Number.MAX_VALUE,
      ),
    ...
    effect: () =>
      ImaginaryUpgrades.totalRebuyables.div(20).add(1).add(
        ImaginaryUpgrades.totalSinglePurchase / 2,
      ),
  ...
  },
  {
    ...
    requirement: () =>
      `Reach a tickspeed of ${
        format("1e75000000000")
      } / sec within Eternity Challenge 5`,
    hasFailed: () => false,
    checkRequirement: () =>
      EternityChallenge(5).isRunning
      && Tickspeed.perSecond.add(1).log(10).gte(7.5e10),
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
    description: () =>
      `Raise all Dimension per-purchase multipliers to ${formatPow(1.5, 0, 1)}`,
    effect: 1.5,
    isDisabledInDoomed: true,
  },
  {
    ...
    checkRequirement: () =>
      player.requirementChecks.reality.maxID1.eq(0)
      && player.antimatter.add(1).log10().gte(1.5e12),
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
    canLock: true,
    ...
    description: () =>
      `${
        Pelle.isDoomed
          ? "Unlock"
          : "Convert Antimatter Dimensions to Continuum and unlock"
  ...
  },
  {
    ...
    hasFailed: () => false,
    checkRequirement: () =>
      Singularity.singularitiesGained.gte(20)
      && Currency.darkEnergy.gte(
        Singularity.cap.mul(
          SingularityMilestone.autoCondense.effectOrDefault(Infinity),
        ),
      ),
  ...
  },
  {
    ...
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
    description: () =>
      `Unlock Autobuyers for repeatable Imaginary Upgrades and generate Imaginary Machines
  ...
  },
  {
    ...
    requirement: () =>
      `Reach ${
        format("1e7400000000000")
      } antimatter with Continuum disabled for the entire Reality`,
    hasFailed: () => !player.requirementChecks.reality.noContinuum,
    checkRequirement: () =>
      player.requirementChecks.reality.noContinuum
    ...
    effect: () =>
      Decimal.clampMin(
        Decimal.pow(Decimal.log10(Currency.imaginaryMachines.value).sub(10), 3),
        1,
      ),
    formatEffect: value => `${formatX(value, 2, 1)}`,
    isDisabledInDoomed: true,
  },
  {
    ...
    hasFailed: () =>
      !Effarig.isRunning || player.requirementChecks.reality.maxGlyphs > -10,
    checkRequirement: () =>
      Effarig.isRunning && player.requirementChecks.reality.maxGlyphs < -10
      && Currency.antimatter.value.add(1).log10().gte(1.5e11),
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
    description: () =>
      `All Glyph Sacrifice totals are increased to ${format(1e100)}`,
    effect: DC.E100,
    isDisabledInDoomed: true,
  },
  {
    ...
    hasFailed: () =>
      !Ra.isRunning || player.requirementChecks.reality.maxGlyphs > 0,
    checkRequirement: () =>
      Ra.isRunning && player.requirementChecks.reality.maxGlyphs <= 0
  ...
  },
  {
    ...
    hasFailed: () =>
      !Ra.isRunning || player.requirementChecks.reality.slowestBH.gt(1e-300),
    checkRequirement: () =>
      Ra.isRunning && player.requirementChecks.reality.slowestBH.lte(1e-300)
  ...
  },
  {
    ...
    hasFailed: () =>
      !Laitela.isRunning || Laitela.maxAllowedDimension !== 0
