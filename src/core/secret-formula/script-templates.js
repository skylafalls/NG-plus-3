  ...
  scripts: [
    {
      name: "Climb EP",
      description:
        `This script performs repeated Eternities, attempting to re-purchase a Time Study Tree every
      ...
      warnings: () => {
        ...
        if (!RealityUpgrade(10).isBought) {
          list.push(
            `This script will be unable to properly set Autobuyer modes without at least ${
              formatInt(100)
            }
            Eternities. Consider getting Reality Upgrade "${
              RealityUpgrade(10).name
            }" before using this at the start
            of a Reality.`,
          );
        }
        ...
        if (!RealityUpgrade(13).isBought) {
          list.push(
            `This template may perform poorly without Reality Upgrade "${
              RealityUpgrade(13).name
            }"`,
          );
        }
        if (!Perk.ttBuySingle.isBought) {
          list.push(
            `This template may perform poorly without Perk "${Perk.ttBuySingle.label}" unless you can generate
    ...
    },
    {
      ...
      description:
        `This script performs repeated fast Eternities after buying a specified Time Study Tree.
      ...
      warnings: () => {
        ...
        if (RealityUpgrade(14).isBought) {
          list.push(
            `You probably do not need to use this due to Reality Upgrade "${
              RealityUpgrade(14).name
            }"`,
          );
        }
        return list;
    ...
    },
    {
      ...
      description:
        `This script buys a specified Time Study Tree and then configures your Autobuyers for gaining
      ...
      warnings: () => {
        ...
        if (!Perk.achievementGroup5.isBought) {
          list.push(
            `You will not start this Reality with Achievement "${
              Achievement(131).name
            }" - grinding
    ...
    },
    {
      ...
      description:
        `This script buys a specified Time Study Tree and then unlocks a specified Eternity Challenge.
      ...
      warnings: () => {
        ...
        if (!Perk.studyECRequirement.isBought) {
          list.push(
            `Eternity Challenges may not be reliably unlockable due to secondary resource requirements, consider
            unlocking Perk "${Perk.studyECRequirement.label}" before using this template`,
          );
        }
        if (!Perk.studyECBulk.isBought) {
          list.push(
            `Using this template without bulk completions of Eternity Challenges may lead to long scripts which
