...
export const devMigrations = {
  patches: [
    ...
    (player) => {
      while (player.celestials.teresa.perkShop.length < 5) {
        player.celestials.teresa.perkShop.push(0);
      }
    },
    (player) => {
    ...
    (player) => {
      ...
      if (player.bestReality === 999999999999) {
        player.bestReality = Number.MAX_VALUE;
      }
      if (player.bestRealityRealTime === 999999999999) {
        player.bestRealityRealTime = Number.MAX_VALUE;
      }
      for (let i = 0; i < 10; i++) {
        if (player.lastTenRealities[i][0] === 2678400000) {
          player.lastTenRealities[i][0] = Number.MAX_VALUE;
        }
        if (player.lastTenRealities[i][3] === 26784000) {
          player.lastTenRealities[i][3] = Number.MAX_VALUE;
        }
      }
    },
    (player) => {
    ...
    (player) => {
      if (!Autobuyer.reality.isUnlocked) {
        player.auto.reality.isActive = false;
      }
    },
    (player) => {
    ...
    (player) => {
      ...
      if (player.reality.perks.has(10)) {
        Perk.startAM.onPurchased();
      }
    },
    (player) => {
    ...
    (player) => {
      ...
      for (const id of Object.keys(player.reality.automator.scripts)) {
        ...
        if (id === player.reality.automator.state.editorScript) {
          selectedID = newID;
        }
        newID++;
      }
    ...
    (player) => {
      if (player.requirementChecks.reality.slowestBH === 0) {
    ...
    (player) => {
      ...
      if (player.auto.eternity.amount.lt(0.01)) {
        player.auto.eternity.amount = new Decimal(0);
      }
    },
    (player) => {
    ...
    (player) => {
      ...
      if (hasDimboost) {
        player.celestials.pelle.upgrades.add(18);
      }
      if (hasDilUpg) {
        player.celestials.pelle.upgrades.add(19);
      }
    },
    (player) => {
    ...
    (player) => {
      ...
      if (player.celestials.pelle.upgrades.delete(20)) {
        player.celestials.pelle.upgrades.add(21);
      }
      if (player.celestials.pelle.upgrades.delete(19)) {
        player.celestials.pelle.upgrades.add(20);
      }
      ...
      for (let i = 9; i >= 7; i--) {
        if (player.celestials.pelle.upgrades.delete(i)) {
          player.celestials.pelle.upgrades.add(i + 1);
        }
      }
      if (hasDimboostsResetNothing) {
        player.celestials.pelle.upgrades.add(7);
      }
    },
    (player) => {
      ...
      if (cel.effarig.quotes) {
        player.celestials.effarig.quoteBits = convToBit(cel.effarig.quotes);
      }
      if (cel.enslaved.quotes) {
        player.celestials.enslaved.quoteBits = convToBit(cel.enslaved.quotes);
      }
      if (cel.v.quotes) {
        player.celestials.v.quoteBits = convToBit(cel.v.quotes);
      }
      if (cel.ra.quotes) {
        player.celestials.ra.quoteBits = convToBit(cel.ra.quotes);
      }
      if (cel.laitela.quotes) {
        player.celestials.laitela.quoteBits = convToBit(cel.laitela.quotes);
      }
      if (cel.pelle.quotes) {
        player.celestials.pelle.quoteBits = convToBit(cel.pelle.quotes);
      }
