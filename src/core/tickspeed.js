...
export function effectiveBaseGalaxies() {
  ...
  replicantiGalaxies = replicantiGalaxies.times(
    1 + TimeStudy(133).effectOrDefault(0)
  ...
  const nonActivePathReplicantiGalaxies = Decimal.min(
  ...
  freeGalaxies = freeGalaxies.mul(
    DC.D1.add(
      Decimal.max(0, Replicanti.amount.max(1).log10().div(1e6))
...
export function getTickSpeedMultiplier() {
  ...
  const effects = Effects.product(
    ...
    GluonUpgrade.redGreen(2),
    GluonUpgrade.redGreen(4),
  );
  if (galaxies.lt(3)) {
    ...
    if (player.galaxies.eq(1)) {
      baseMultiplier = DC.C1D1_11888888;
    }
    if (player.galaxies.eq(2)) {
      baseMultiplier = DC.C1D1_11267177;
    }
    if (NormalChallenge(5).isRunning) {
      baseMultiplier = DC.D1.div(1.08);
      if (player.galaxies.eq(1)) {
        baseMultiplier = DC.D1.div(1.07632);
      }
      if (player.galaxies.eq(2)) {
        baseMultiplier = DC.D1.div(1.072);
      }
    }
...
export function buyMaxTickSpeed() {
  ...
  if (NormalChallenge(9).isRunning) {
    ...
    const purchases = Tickspeed.costScale.getMaxBought(
    ...
    if (purchases !== null) {
      if (
        purchases.logPrice.eq(player.antimatter.max(1).log10())
        && player.dimensions.antimatter[0].amount.eq(0)
      ) {
        purchases.logPrice = Tickspeed.costScale.calculateCost(
          purchases.quantity.sub(1),
        );
    ...
    for (
      let i = 0;
      i < 5
      && (player.antimatter.neq(Tickspeed.cost)
        && player.dimensions.antimatter[0].amount.neq(0));
      i++
    ) {
      buyTickSpeed();
    }
...
export const Tickspeed = {
  get isUnlocked() {
    return Achievement(12).isUnlocked
  ...
  get cost() {
    return this.costScale.calculateCost(
      player.totalTickBought.add(player.chall9TickspeedCostBumps),
    );
  },
  ...
  get totalUpgrades() {
    ...
    if (Laitela.continuumActive) {
      boughtTickspeed = new Decimal(this.continuumValue);
    } else {
      boughtTickspeed = new Decimal(player.totalTickBought);
    }
...
export const FreeTickspeed = {
  ...
  get multToNext() {
    if (this.amount.lt(this.softcap)) {
  ...
  fromShards(shards) {
    ...
    return {
      newAmount: quant.quantity,
      nextShards: this.tickExpo.calculateCost(quant.quantity.add(1)),
    };
  },
};
